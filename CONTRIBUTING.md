...
Keyboard tests must validate pure down/up plans without posting input to an uncontrolled desktop. Cover mode isolation, trust revocation, switch reversibility, AUTO staying Normal, invalid keys/counts and complete modifier release. Native end-to-end input tests need an explicitly controlled target window; do not run arbitrary configured shortcuts in CI. Keep the shared key catalog and both platform emitters consistent. See [Action layers](docs/ACTION_LAYERS.md) for behavior and limits.

Recorder changes also need tests for modifier-only taps, held-key auto-repeat, incomplete/cancelled chords, event/time limits, legacy configuration migration and mode IDs surviving reorder. Never add global hooks or persist raw captured events without an explicit security/design review. The Windows local adapter uses the existing winit backend via Slint's version-specific `unstable-winit-030` API; revalidate it before changing the constrained Slint minor version.

For a Windows-client sample-data layout render, run `watchbridge-windows --snapshot <new-file.bmp> --keyboard --minimum-size`. Omit `--keyboard` for the Actions page; use `--cards` or `--second-row` for its scrolled card rows. Rendering returns before state loading, BLE startup and action callbacks; it briefly shows the sample window. Output is the Slint client area only, never the desktop, and existing files are not overwritten. Native CI converts these BMPs to PNG review artifacts with Windows' image encoder. On macOS, use the Swift app's `--snapshot <directory>` tooling instead.

Hosted Windows layout checks use `cargo build --locked --release --package watchbridge-windows --features software-preview` and set `SLINT_BACKEND=winit-software` for that fixture process. This optional CPU renderer avoids requiring a GPU context/readback in CI. Distributed review packages are built first without this feature; their production renderer is unchanged. CPU layout previews do not certify GPU rendering or Mica.

Add `--manual-key` to the keyboard fixture to inspect the expanded controls; Windows also has `--mode-editor` for the color/name dialog. macOS uses `--only keyboard --manual-key` and `--only indicator` for these new views. Fixtures never install recorder callbacks or start input capture. Inspect default/minimum previews before publishing screenshot changes.

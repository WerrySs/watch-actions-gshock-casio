...
struct WatchBridgeApp: App {
    @State private var modeIndicator = ModeIndicatorController()
    ...
    var body: some Scene {
        WindowGroup("WatchBridge") {
            ContentView()
                .environment(store)
                .onAppear { modeIndicator.start(store) }
        ...
        MenuBarExtra {
            ...
            Image(nsImage: ModeStyle.menuSymbol(connected: store.isConnected, color: store.config.color(for: store.panelActionLayer)))
                .renderingMode(.original)
                .help("WatchBridge · \(store.config.name(for: store.panelActionLayer)) mode")
        }

...
The formats differ between platforms: there is no supported direct state-file interchange. Rust state schema 2 adds per-watch queues; macOS uses `pending-by-watch-v2.json`. Old unscoped pending entries are retained in their legacy file/field and are never automatically assigned to a watch. Recreate intended changes explicitly.

...
The formats differ between platforms: there is no supported direct state-file interchange. Rust state schema 2 added per-watch queues; development schema 3 adds action layers and keyboard shortcuts while preserving old settings. macOS uses `pending-by-watch-v2.json` for queued changes and a separate Codable action configuration. Old unscoped pending entries are retained in their legacy file/field and are never automatically assigned to a watch. Recreate intended changes explicitly. Back up the data folder before testing development builds; downgrades are unsupported and older macOS builds may discard new action fields. See [Action layers](ACTION_LAYERS.md).
